// CONFIGURACIÓN DE TU API DE GEMINI (Capa Gratuita)
...
let currentScanResult = null; // Almacena el último escaneo exitoso

// 1. Alternar vistas sin recargar
function switchView(viewId) {
    document.querySelectorAll('.view').forEach(view => view.classList.remove('active'));
    document.getElementById(viewId).classList.add('active');
    
    // Si entra al escáner, inicia la cámara
...
// 4. Capturar desde el feed de video
...
// 5. Cargar desde la galería del dispositivo
...
// 6. Preparar UI y mandar datos a procesar
...
// Resetear UI para nuevo intento
...
// 7. Conexión a Gemini Vision API
async function enviarAGemini(base64Image) {
    ...
    Cruza la información visual con el GPS de forma estricta para determinar con precisión lagos, ríos, volcanes o monumentos nicaragüenses específicos (por ejemplo: saber si es el volcán Masaya, cerro negro, río san juan, etc.).
    ...
    try {
        ...
        // Guardar temporalmente en memoria para opción de archivado
        currentScanResult = {
            id: Date.now(),
        ...
        resultText.innerHTML = "<p><strong>Error:</strong> No se pudo conectar con el servidor de análisis. Verifica tu API Key y conexión.</p>";
        resultContainer.style.display = "block";
    } finally {
...
// 8. Sistema de guardado temporal (LocalStorage)
btnSaveCurrent.addEventListener('click', () => {
    ...
    btnSaveCurrent.style.display = "none"; // Ocultar tras guardar
});
